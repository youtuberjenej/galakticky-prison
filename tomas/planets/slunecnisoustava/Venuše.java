...
public class Venuše extends Planet {
    @Override
        ...
        for(int x = mine().get(0); x <= mine().get(1); x++) {
            for(int y = mine().get(2); y <= mine().get(3); y++) {
                for(int z = mine().get(4); z <= mine().get(5); z++) {
                    blocks++;
                    if(!(Bukkit.getWorld("VENUSE").getBlockAt(x, y, z).getType().equals(Material.AIR))) blocksInMine++;
                }
            }
        }
    ...
    public String message() {
        return ChatColor.AQUA + "Vítej na planetě VENUSE!";
    }
    ...
    public void reset() {
        ...
        for(World world : Bukkit.getWorlds()) {
            if(world.getName().equals("VENUSE")) contains = true;
        }
        if(!contains) return;
        Main.instance.broadcast(ChatColor.AQUA + "důl VENUSE byl obnoven!");
        for(int x = mine().get(0); x <= mine().get(1); x++) {
            for(int y = mine().get(2); y <= mine().get(3); y++) {
                for(int z = mine().get(4); z <= mine().get(5); z++) {
                    ...
                    for(Material material : chance().keySet()) {
                        if(number >= n1 && number <= n1 + chance().get(material)) {
                            Bukkit.getWorld("VENUSE").getBlockAt(x, y, z).setType(material);
                        }
        ...
        for(Player player : Bukkit.getOnlinePlayers()) {
            if(player.getWorld().getName().equals("VENUSE")) {
    ...
    public HashMap<Material, Integer> chance() {
        ...
        chances.put(Material.STONE, 75);
        chances.put(Material.COAL_ORE, 20);
        chances.put(Material.IRON_ORE, 5);
        return chances;
    }
    ...
    public Location spawnLocation() {
        return new Location(Bukkit.getWorld("VENUSE"), 0, 65, 0);
    }
}
