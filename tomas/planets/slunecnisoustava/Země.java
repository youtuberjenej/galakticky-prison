...
public class Země extends Planet {
    @Override
        ...
        for(int x = mine().get(0); x <= mine().get(1); x++) {
            for(int y = mine().get(2); y <= mine().get(3); y++) {
                for(int z = mine().get(4); z <= mine().get(5); z++) {
                    blocks++;
                    if(!(Bukkit.getWorld("ZEME").getBlockAt(x, y, z).getType().equals(Material.AIR))) blocksInMine++;
                }
            }
        }
    ...
    public String message() {
        return ChatColor.AQUA + "Vítej na planetě ZEME!";
    }
    ...
    public void reset() {
        ...
        for(World world : Bukkit.getWorlds()) {
            if(world.getName().equals("ZEME")) contains = true;
        }
        if(!contains) return;
        Main.instance.broadcast(ChatColor.AQUA + "důl ZEME byl obnoven!");
        for(int x = mine().get(0); x <= mine().get(1); x++) {
            for(int y = mine().get(2); y <= mine().get(3); y++) {
                for(int z = mine().get(4); z <= mine().get(5); z++) {
                    ...
                    for(Material material : chance().keySet()) {
                        if(number >= n1 && number <= n1 + chance().get(material)) {
                            Bukkit.getWorld("ZEME").getBlockAt(x, y, z).setType(material);
                        }
        ...
        for(Player player : Bukkit.getOnlinePlayers()) {
            if(player.getWorld().getName().equals("ZEME")) {
    ...
    public HashMap<Material, Integer> chance() {
        ...
        chances.put(Material.STONE, 65);
        chances.put(Material.COAL_ORE, 25);
        chances.put(Material.IRON_ORE, 8);
        chances.put(Material.GOLD_ORE, 2);
        return chances;
    }
    ...
    public Location spawnLocation() {
        return new Location(Bukkit.getWorld("ZEME"), 0, 65, 0);
    }
}
